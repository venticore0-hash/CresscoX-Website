(function () {
  const config = {
    apiBaseUrl: window.CX_CHATBOT_API_BASE_URL || 'https://cresscox-chatbot.onrender.com',
  ...
  root.innerHTML = `
    ...
    <section class="cx-chatbot-window" aria-label="${escapeHtml(config.assistantName)}">
      ...
      <footer class="cx-chatbot-footer">
        ...
        <div class="cx-chatbot-note">For best results, connect this widget to your backend with your Gemini API key on the server only.</div>
      </footer>
    </section>
  `;
  ...
  formEl.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      renderAssistantMessage(
        `I’m having trouble reaching the AI backend right now. Please check that your Node server is running and that GEMINI_API_KEY is set correctly.\n\nTechnical note: ${error.message}`
      );
    } finally {
